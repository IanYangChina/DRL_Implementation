...
class SAC(Agent):
    def __init__(self, algo_params, env, transition_tuple=None, path=None, seed=-1):
        ...
        self.network_dict.update({
            ...
            'critic_1_target': CriticPointNet(output_dim=1, action_dim=self.action_dim).to(self.device),
            'critic_2': CriticPointNet(output_dim=1, action_dim=self.action_dim).to(self.device),
            'critic_2_target': CriticPointNet(output_dim=1, action_dim=self.action_dim).to(self.device),
    ...
    def run(self, test=False, render=False, load_network_ep=None, sleep=0):
        ...
        for ep in range(num_episode):
            ...
            if (ep % self.testing_gap == 0) and (ep != 0) and (not test):
                ...
                self.logger.add_scalar(tag='Task/test_return', scalar_value=(sum(ep_test_return) / self.testing_episodes), global_step=ep)
