...
return_ddpg_her = json.load(open(os.path.join(path, 'exp_continuous_gym', 'ddpg', 'return_statistic.json')))
return_ddpg_pher = json.load(open(os.path.join(path, 'exp_continuous_gym', 'ddpg_per', 'return_statistic.json')))
return_sac_her = json.load(open(os.path.join(path, 'exp_continuous_gym', 'sac', 'return_statistic.json')))
return_sac_pher = json.load(open(os.path.join(path, 'exp_continuous_gym', 'sac_per', 'return_statistic.json')))

plot.smoothed_plot_mean_deviation(os.path.join(path, 'src', 'returns_half_cheetah.png'),
                                  [return_ddpg_her, return_ddpg_pher, return_sac_her, return_sac_pher],
                                  legend=['DDPG', 'DDPG PER', 'SAC', 'SAC PER'],
                                  legend_loc='lower right',
                                  title='Pybullet Half Cheetah',
                                  window=10,
